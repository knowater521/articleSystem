...
Command.prototype = {
	...
	_phonetic:function (phoneticContent) {
		const editor = this.editor;
		let range = editor.selection.getRange();
		let content = range.toString();
		let editorDom = document.getElementById(editor.textElemId);
		let paragraphDoms = Array.from(editorDom.querySelectorAll("p"));
		editorDom.contentEditable = "false";
		paragraphDoms.forEach((item)=>{
			item.contentEditable = "true";
		},this);
		let html = `<ruby contentEditable="false">${content}<rt>${phoneticContent}</rt></ruby><span></span>`;
		this._execCommand('insertHTML', html);
